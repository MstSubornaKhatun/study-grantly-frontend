...
// ✅ 2. Frontend Component এ date display এর জন্য helper function
import React, { useEffect, useState } from "react";
import Swal from "sweetalert2";
import useAxiosSecure from "../../../hooks/useAxiosSecure";
import { Eye, Trash2, CheckCircle2, XCircle, Ban } from "lucide-react";
...
const ManageAppliedApplications = () => {
  ...
  const formatDate = (dateString) => {
    ...
    try {
      ...
      // বা আমেরিকান format চাইলে
      // return date.toLocaleDateString('en-US');
      
      // বা custom format চাইলে
      // return date.toLocaleDateString('en-US', {
      //   year: 'numeric',
      //   month: 'short',
      //   day: 'numeric'
                ...
                {applications.length === 0 ? (
                  ...
                  applications.map((app, idx) => (
                    ...
                    >
                      ...
                      {/* ✅ Fixed date display - Using appliedDate */}
                      <td className="px-6 py-4 text-sm text-gray-700 dark:text-gray-300">
                        <div className="flex flex-col">
                          <span className="font-medium">
                            {formatDate(app.appliedDate || app.applicationDate || app.createdAt)}
                          </span>
                          {/* Optional: Show time as well */}
                          {(app.appliedDate || app.applicationDate || app.createdAt) && (
                            <span className="text-xs text-gray-500 dark:text-gray-400">
                              {new Date(app.appliedDate || app.applicationDate || app.createdAt).toLocaleTimeString('en-GB', {
                                hour: '2-digit',
                                minute: '2-digit'
                              })}
                            </span>
                      ...
                      <td className="px-6 py-4">
                        <div className="flex gap-2 justify-center flex-wrap">
                          <button
                            className="inline-flex items-center justify-center w-9 h-9 rounded-lg bg-green-600 hover:bg-green-700 text-white transition-all duration-200 hover:shadow-lg disabled:opacity-50 disabled:cursor-not-allowed"
                            disabled={app.status !== "pending"}
                            onClick={() =>
                              handleStatusUpdate(app._id, "approved")
                            }
                          ...
                          <button
                            className="inline-flex items-center justify-center w-9 h-9 rounded-lg bg-red-600 hover:bg-red-700 text-white transition-all duration-200 hover:shadow-lg disabled:opacity-50 disabled:cursor-not-allowed"
                            disabled={app.status !== "pending"}
                            onClick={() =>
                              handleStatusUpdate(app._id, "rejected")
                            }
                          ...
                          <button
                            className="inline-flex items-center justify-center w-9 h-9 rounded-lg bg-orange-600 hover:bg-orange-700 text-white transition-all duration-200 hover:shadow-lg disabled:opacity-50 disabled:cursor-not-allowed"
                            disabled={app.status !== "pending"}
                            onClick={() =>
                              handleStatusUpdate(app._id, "cancelled")
                            }
                          ...
                          <button
                            title="Delete"
                            className="inline-flex items-center justify-center w-9 h-9 rounded-lg bg-gray-600 hover:bg-gray-700 text-white transition-all duration-200 hover:shadow-lg"
                            onClick={() => handleDelete(app._id)}
                          >
                            <Trash2 size={16} />
                          </button>
